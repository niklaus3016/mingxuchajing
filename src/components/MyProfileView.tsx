...
export const MyProfileView: React.FC<MyProfileViewProps> = ({
  ...
  return (
    <div className="space-y-5 pb-20 animate-fade-in" id="profile_view_viewport">
      ...
      <div 
        className="rounded-2xl p-6 bg-linear-to-r from-emerald-800 to-emerald-950 dark:from-emerald-900 dark:to-stone-900 border border-emerald-900 text-white shadow-md relative overflow-hidden"
        id="profile_top_user_badge"
      >
        <div className="relative z-10 flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-white/10 dark:bg-stone-800/40 border-2 border-white/20 backdrop-blur-md flex items-center justify-center font-serif text-2xl font-bold">
            茗
          </div>
          <div className="space-y-1">
            <h2 className="font-serif text-lg font-bold tracking-wide">雅鉴茶人</h2>
            <p className="text-[11px] text-emerald-250 italic">
              茗香有序，茶经随心
            </p>
          </div>
        </div>
      ...
      <div 
        className="grid grid-cols-3 gap-1 p-1 bg-stone-100 dark:bg-stone-900 rounded-xl text-center text-xs font-medium"
        id="profile_sub_menu"
      >
        {[
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 py-2.5 rounded-lg transition-all duration-200 ${
                matched
                  ? 'bg-white dark:bg-stone-800 text-emerald-850 dark:text-emerald-400 font-bold shadow-3xs'
        ...
        <div className="space-y-4 animate-fade-in animate-duration-200" id="profile_tab_favs">
          ...
          <div className="relative" id="fav_search_bar_container">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 rounded-xl text-xs bg-white dark:bg-stone-900 border border-stone-150 dark:border-stone-800 text-stone-800 dark:text-stone-100 focus:outline-hidden"
            />
          </div>
          ...
          <div className="space-y-4" id="favs_list_wrapper">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center px-1">
                <span className="font-serif text-button text-xs font-bold text-stone-800 dark:text-stone-200 flex items-center gap-1">
                  <span className="w-1.5 h-3.5 bg-emerald-700 dark:bg-emerald-500 rounded-full"></span>
                  收藏的茶品 ({filteredFavTeas.length} 款)
                </span>
              </div>

              {filteredFavTeas.length === 0 ? (
                <div className="text-center py-6 px-4 rounded-xl border border-dashed border-stone-200 dark:border-stone-800 bg-stone-50/50 text-stone-400 text-xs">
                ...
                <div className="grid grid-cols-1 gap-2" id="fav_teas_deck">
                  {filteredFavTeas.map(tea => {
                    return (
                      <div
                        key={tea.id}
                        className="p-3 bg-white dark:bg-stone-900 rounded-lg border border-stone-150 dark:border-stone-800 flex justify-between items-center group hover:border-emerald-600/30 transition-all duration-200"
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-1.5">
                            <span className="text-[9px] bg-stone-100 dark:bg-stone-800 text-stone-500 px-1.5 py-0.2 rounded">
                              {tea.category}
                            </span>
                            <span className="text-xs font-medium text-stone-800 dark:text-stone-205 group-hover:text-emerald-700 transition-colors">
                              {tea.name}
                            </span>
                          </div>
                          <p className="text-[10px] text-stone-400 dark:text-stone-350 flex items-center gap-0.5 truncate max-w-xs">
                        ...
                        <button
                          ...
                          className="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-full transition-colors"
                          title="取消收藏"
                        >
            ...
            {(favoritedTeas.length > 0 || favoritedKnowledge.length > 0) && (
              <div className="pt-4 border-t border-dashed border-stone-200 dark:border-stone-800 flex justify-center">
                {showClearFavsConfirm ? (
                  <div className="p-3 bg-rose-500/5 rounded-xl border border-rose-500/10 text-center w-full max-w-xs space-y-2">
                    <p className="text-[10px] text-rose-850 dark:text-rose-400 font-semibold">确认要批量清空全部收藏的数据吗？此操作无法挽回</p>
                    <div className="flex gap-2 justify-center">
                      <button
                        onClick={() => setShowClearFavsConfirm(false)}
                        className="px-3 py-1 bg-stone-200 hover:bg-stone-300 text-stone-700 text-[10px] font-bold rounded"
                      >
                        保留
                      </button>
                      <button
      ...
      {activeSegment === 'settings' && (
        <div className="space-y-4 animate-fade-in animate-duration-200" id="profile_tab_settings">
          <div className="p-4 rounded-xl bg-white dark:bg-stone-900 border border-stone-150 dark:border-stone-800 space-y-4 shadow-3xs">
            <h3 className="font-serif text-sm font-bold text-stone-850 dark:text-stone-100 pb-2 border-b border-stone-100 dark:border-stone-850">
              设置偏好状态
            </h3>
            ...
            <div className="flex justify-between items-center py-1">
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-stone-800 dark:text-stone-200">禅钟冲完铃声及按键反馈音</span>
                <p className="text-[10px] text-stone-400 dark:text-stone-500">计时结束或按钮交互时播放温馨音律</p>
              </div>
              ...
              <button
                ...
                className={`py-1.5 px-3.5 rounded-lg text-xs font-bold font-serif flex items-center gap-1 border transition-all ${
                  settings.soundEnabled
                    ...
                    : 'bg-stone-50 text-stone-405 border-stone-200'
                }`}
                id="btn_toggle_audio"
              >
                {settings.soundEnabled ? (
            ...
            <div className="flex justify-between items-center py-1 border-t border-stone-100 dark:border-stone-850 pt-3">
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-stone-800 dark:text-stone-200">应用隐私政策条款</span>
                <p className="text-[10px] text-stone-400 dark:text-stone-500">查阅关于本应用的数据存储与自主权说明</p>
              </div>
              ...
              <button
                ...
                className="py-1.5 px-3.5 rounded-lg text-xs font-bold font-serif flex items-center gap-1.5 border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-900 text-stone-700 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 transition-all shadow-3xs"
                id="btn_view_privacy"
              >
      ...
      {activeSegment === 'about' && (
        <div className="space-y-4 animate-fade-in animate-duration-200" id="profile_tab_about">
          <div className="p-6 rounded-2xl bg-white dark:bg-stone-900 border border-stone-150 dark:border-stone-800 space-y-4 shadow-sm text-center">
            
            <div className="flex justify-center mb-1">
              <div className="w-16 h-16 rounded-2xl bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-serif text-3xl font-extrabold shadow-3xs border border-emerald-100 dark:border-emerald-900/60">
                学
              </div>
            </div>

            <div className="space-y-1">
              <h3 className="font-serif text-lg font-bold text-stone-850 dark:text-stone-100">茗序茶经</h3>
              <p className="text-xs text-stone-400 font-mono">版本：v1.0</p>
            </div>

            <p className="text-stone-605 dark:text-stone-350 text-xs leading-relaxed text-left p-4 bg-stone-50 dark:bg-stone-950/30 rounded-xl border border-stone-110 border-dashed">
